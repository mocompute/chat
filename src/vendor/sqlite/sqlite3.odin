...
foreign sqlite3 {
	...
	@(require_results) initialize :: proc() -> Result ---
	@(require_results) config :: proc(option: Config, #c_vararg args: ..any) -> Result ---

	@(require_results) open :: proc(filename: cstring, ppDb: ^Connection) -> Result ---
	@(require_results) open_v2 :: proc(filename: cstring, ppDb: ^Connection, flags: c.int, zVfs: cstring) -> Result ---
	close :: proc(db: Connection) -> Result ---

	@(require_results) exec :: proc(db: Connection, sql: cstring, cb: Exec_Callback, ud: rawptr, errsmg: ^cstring) -> Result ---
	@(require_results) prepare_v2 :: proc(db: Connection, zSql: cstring, nByte: c.int, ppStmt: ^Statement, pzTail: ^cstring) -> Result ---
	@(require_results) step :: proc(stmt: Statement) -> Result ---
	@(require_results) reset :: proc(stmt: Statement) -> Result ---
	finalize :: proc(stmt: Statement) -> Result ---

	// bind_* use 1-based parameter indices from left to right
	@(require_results) bind_blob:: proc(stmt: Statement, i_1: c.int, p: rawptr, n: c.int, cb: Bind_Callback) -> Result ---
	@(require_results) bind_blob64:: proc(stmt: Statement, i_1: c.int, p: rawptr, n: u64, cb: Bind_Callback) -> Result ---
	@(require_results) bind_double:: proc(stmt: Statement, i_1: c.int, d: c.double) -> Result ---
	@(require_results) bind_int :: proc(stmt: Statement, i_1: c.int, val: c.int) -> Result ---
	@(require_results) bind_int64 :: proc(stmt: Statement, i_1: c.int, val: i64) -> Result ---
	@(require_results) bind_null :: proc(stmt: Statement, i_1: c.int) -> Result ---
	@(require_results) bind_text :: proc(stmt: Statement, i_1: c.int, s: cstring, n: c.int, cb: Bind_Callback) -> Result ---
	@(require_results) bind_text16 :: proc(stmt: Statement, i_1: c.int, s: cstring, n: c.int, cb: Bind_Callback) -> Result ---
	@(require_results) bind_text64 :: proc(stmt: Statement, i_1: c.int, s: cstring, n: u64, cb: Bind_Callback, encoding: u8) -> Result ---
	@(require_results) bind_value :: proc(stmt: Statement, i_1: c.int, value: Value) -> Result ---
	@(require_results) bind_pointer :: proc(stmt: Statement, i_1: c.int, p: rawptr, s: cstring, cb: Bind_Callback) -> Result ---
	@(require_results) bind_zeroblob :: proc(stmt: Statement, i_1: c.int, n: c.int) -> Result ---
	@(require_results) bind_zeroblob64 :: proc(stmt: Statement, i_1: c.int, n: u64) -> Result ---
